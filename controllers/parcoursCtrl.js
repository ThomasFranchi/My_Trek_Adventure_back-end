...
const parcoursCtrl = 
{
    ...
    {
        const {name, duration, description, price, picture, difficulty} = req.body;

        // Check if evertyhing is good (typeof, unemptiness, regexp validation)
        if (typeof(name) !== "string" || typeof(description) !== "string" || typeof(picture) !== "string")
        {
            return res.status(422).json({message: "Un ou plusieurs champs ne sont pas du bon type"});
        }
        if (name === "" || duration === "" || description === "" || picture === "")
        {
