...
const parcoursCtrl = {
  ...
  async getSingleParcoursById(req, res) {
    const parcours = await parcoursModel.findOne(
      { _id: req.params.id }).exec();
    if (!parcours) {
      return res.status(422).json(
        { message: "L'opération n'a pas pu être effectuée" });
    }
    return res.json(parcours);
  },
  // Get a single parcours, according to its id 
  ...
  async duoFilter(req, res) {
    ...
    if (req.params.price > 0 && req.params.difficulty > 0) {
      ...
      parcours = await parcoursModel.find(
        { difficulty: { $eq: req.params.difficulty }, 
      ...
      if (req.params.price > 0) {
        parcours = await parcoursModel.find(
          { price: { $lte: req.params.price } }).exec();
      }
      // If price = 0 and difficulty > 0
