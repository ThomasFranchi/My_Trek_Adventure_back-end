...
const bookingsCtrl = {
  ...
  async getMyBookings(req, res) {
    ...
    const bookings = await treksModel.find ({bookings:{$elemMatch:{userID: userID}}}, 
      "beginDate endDate parcoursID guideID minPlaces maxPlaces trekName trekState slug").exec();
    if (!bookings)
    {
      return res.status(422).json({message:"L'opération n'a pas pu être effectuée"});
    }
